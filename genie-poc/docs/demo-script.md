...
| **Native** | She asks; the app changes | Same Riise. Native adds tabs / screens, then asks the next step (keep as default, or buy). She confirms. |
| **Trading agent** | The situation starts | Pulse + one picture + optional tickets already there. Ask is a back pocket. She confirms. |

Line to leave them with: Traditional = she seeks. AI-enabled = she asks. Native = she asks and the **app** rearranges. Trading agent = the system anticipates. **She keeps authority.**
...
2. **09:15 Traditional → AI-enabled → Trading agent** (5 min) — quiet open is the trap.
3. **Native, any clock** (3 min) — same Riise as AI-enabled. Chip **Add watchlist tab**: tab opens, Native asks **Keep as default?** (pin) vs **Only this session**. Then **PE < 50, ROE > 30**: Screener opens and Native asks **Buy any of these?** Tick two names → **Fill order for selected**. Orders shows a filled CNC LIMIT ticket. She Confirm.
4. **09:42 Trading agent, then glance Traditional** (3 min) — the plan printed.
5. **One more Trading agent clock** — **11:20** or **14:05**, then **15:31** (no ticket).
