...
export default function CopilotView({
  ...
  const screener = extraTabs.find((t) => t.kind === "screener");

  const applyWorkspace = (event: WorkspaceEvent) => {
    if (event.action === "add" && event.tab) {
      const next = event.tab;
      const existing = extraTabs.find((t) => t.kind === next.kind || t.id === next.id);
      const merged = {
        ...next,
      ...
      setExtraTabs((prev) => {
        const rest = prev.filter((t) => t.kind !== merged.kind && t.id !== merged.id);
        return [...rest, merged];
      });
      setFocusTab(next.id);
      if (next.kind === "screener") {
        setSelected([]);
        setPendingTab(merged);
        setFollow("buy");
    ...
    if (event.action === "pin") {
      ...
      setExtraTabs((prev) =>
        prev.map((t) => (t.kind === kind || t.id === event.which ? { ...t, pinned: true, closable: true } : t)),
      );
      setFollow(null);
    }
    if (event.action === "unpin") {
      const kind = kindFromWhich(event.which);
      setExtraTabs((prev) =>
        prev.map((t) => (t.kind === kind || t.id === event.which ? { ...t, pinned: false, closable: true } : t)),
      );
    }
    if (event.action === "remove") {
      ...
      setExtraTabs((prev) => {
        ...
        return prev.filter((t) => t.kind !== kind && t.id !== which);
      });
      setFocusTab("explore");
      setSelected([]);
      setFollow(null);
  ...
  return (
    <div className="enabled">
      <ClassicView
        ...
        hideWatcherDrafts
        emptyOrders="No orders yet. Ask Copilot to fill a ticket — then confirm."
        news={news}
        extraTabs={extraTabs}
        openTab={focusTab}
        onCloseTab={(id) => {
        ...
        <AskAgent
          ...
          onChipUsed={(label) => {
            if (label === "Keep as default" && pendingTab) {
              setExtraTabs((prev) =>
                prev.map((t) =>
                  t.id === pendingTab.id || t.kind === pendingTab.kind
                    ? { ...t, pinned: true, closable: true }
                    : t,
                ),
              );
              setFollow(null);
