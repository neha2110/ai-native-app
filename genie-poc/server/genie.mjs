...
const COPILOT_EXTRA = `
You are in Native mode. The trader is looking at the Riise app. You can CHANGE THE APP, not only the chat.

Cash equity screens, named watchlists, extra tabs, and "similar" asks (highest ROE, top gainers, banks only, a new watchlist of these names) are in scope. Do not refuse them or steer back to the F&O book.

First action, then a second-level ask. Always do both.

- To create a Watchlist (including a NEW one while another is open), use add_workspace_tab with kind=watchlist. Every call must create a new tab. Set title (e.g. "Banks", "Low PE"). Pass symbols when they named scrips or when you just screened names. Use watchlist_history if they want plots. Then ask: keep this tab after the session as a default, or only for now? If they want it as default, pin_workspace_tab with the new tab's id. Do not dump the table in chat.
- Default only means it comes back next session. They can close any tab you created. remove_workspace_tab can take a tab id or a kind, or all_optional. Never remove Explore, Portfolio, Positions, Option Chain, or Orders.
- For any screen (lowest PE, lowest PB, highest ROE, gainers, sector, name search), use screen_equities with the matching filters and sortBy/limit. Lowest PE → pe_asc. Lowest PB → pb_asc. Highest ROE → roe_desc. Top gainers → change_desc. Then ask if they want to buy, or to save the names as a NEW watchlist (add_workspace_tab with those symbols).
- When they want to buy, use draft_equity_orders. That FILLS the Orders tab. They still Confirm. Never say the order was placed.
...
const COPILOT_TOOLS = [
  {
    ...
    description:
      "Create a NEW optional tab on the Riise app (never in chat). Each call adds another tab — it does not replace an existing watchlist. Pass title and symbols to fill a named list.",
    parameters: {
      type: "object",
      properties: {
        kind: {
        ...
        title: { type: ["string", "null"], description: "Tab name, e.g. Low PE or Banks." },
        symbols: {
          type: ["array", "null"],
          items: { type: "string" },
          description: "NSE symbols to put on the tab. Null uses a default basket.",
        },
      },
      required: ["kind", "title", "symbols"],
      additionalProperties: false,
    },
    strict: true,
  },
  {
    ...
    parameters: {
      ...
      properties: {
        which: {
          type: "string",
          description: "Tab id from the last add (preferred), or kind: watchlist, watchlist_history, performance, screener.",
        },
      },
      required: ["which"],
      additionalProperties: false,
  ...
  },
  {
    ...
    description:
      "Remove a Native-created tab, including one marked Default. Does not remove core Riise tabs (Explore, Portfolio, Positions, Option Chain, Orders).",
    parameters: {
      type: "object",
      properties: {
        which: {
          type: "string",
          description:
            "Tab id, or kind (watchlist, watchlist_history, performance, screener), or all_optional to clear extras.",
        },
      },
      required: ["which"],
      additionalProperties: false,
  ...
  },
  {
    ...
    description:
      "Screen the mocked cash-equity universe. Use this for any list-like ask: lowest/highest PE or PB, ROE, gainers/losers, sector, name search. Sort and limit as asked. Opens a new Screener tab.",
    parameters: {
      type: "object",
      properties: {
        peMax: { type: ["number", "null"], description: "Keep names with PE below this, e.g. 50." },
        peMin: { type: ["number", "null"] },
        pbMax: { type: ["number", "null"], description: "Keep names with price-to-book below this, e.g. 3." },
        pbMin: { type: ["number", "null"] },
        roeMin: { type: ["number", "null"], description: "Keep names with ROE above this percent, e.g. 30." },
        roeMax: { type: ["number", "null"] },
        sector: { type: ["string", "null"], description: "IT, Banks, Energy, Consumer, Auto, Pharma, etc." },
        query: { type: ["string", "null"], description: "Name or symbol text match." },
        changePctMin: { type: ["number", "null"], description: "Min day change %, e.g. 1 for gainers." },
        changePctMax: { type: ["number", "null"] },
        sortBy: {
          type: ["string", "null"],
          enum: [
            "pe_asc",
          ...
          description: "pe_asc = lowest PE. pb_asc = lowest PB. change_desc = top gainers.",
        },
        limit: {
          type: ["integer", "null"],
          description: "Max names after sort, e.g. 10.",
        },
        title: { type: ["string", "null"], description: "Screener tab title." },
      },
      required: [
        "peMax",
        "peMin",
...
function runTool(name, args) {
  switch (name) {
    ...
    case "add_workspace_tab":
      return workspaceTab(args?.kind, { title: args?.title, symbols: args?.symbols });
    ...
    case "screen_equities":
      return screenEquities({
        ...
        sector: args?.sector,
        query: args?.query,
        changePctMin: args?.changePctMin,
        changePctMax: args?.changePctMax,
        sortBy: args?.sortBy,
        limit: args?.limit,
        title: args?.title,
      });
